...
def BGMCA(X,n=2,maxts = 7,mints=3,nmax=100,q_f=0.1,L0=0,UseP=1,verb=0,Init=0,Aposit=False,BlockSize= None,NoiseStd=[],IndNoise=[],\
...
#-------------- Thresholding
                ...
                for r in indS:
                    ...
                    # w_FM[r,iB] = np.sum(St**2)/(len(St)*1.)
...
#------ Fusion of mixing matrices A
        if numBlocks == 1:
            ...
            if weightFMOpt != 1:
                ...
                # For didactiv purposes
                w_FM = np.ones(w_FM.shape)
                w_FM = np.multiply(w_FM,w_Ai)
