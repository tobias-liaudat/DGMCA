...
# from bgmca9 import GMCA as gmca_block
# from bgmca9_bisbis import GMCA as gmca_block9_bisbis
# from bgmca9_bis import GMCA as gmca_block9_bis
# from bgmca7_bis import GMCA as gmca_block_bis
from GMCA import GMCA as gmca_original_SC
from BLGMCA3 import BLGMCA as blgmca3
from BLGMCA4 import BLGMCA as blgmca4
from BLGMCA2_bis import BLGMCA as blgmca2_bis
from BLGMCA2_bisbis import BLGMCA as blgmca2_bisbis
from BLGMCA import BLGMCA as blgmca
import copy as cp
...
divisors = np.array([500])
lineBlockSize = np.array([5])#,8,10,20])#,12,14,16,18,20,25])#5, 6, 7, 8, 9, 10,20])
...
ca_final_sB1 = np.zeros([2,len(lineBlockSize),numIts])
ca_final_sB2 = np.zeros([2,len(lineBlockSize),numIts])
ca_final_sB3 = np.zeros([2,len(lineBlockSize),numIts])
...
for it_n in tqdm(range(numIts)):
    ...
    # time1 = time.time()
    # Results_sB0 = gmca_block_bis(cp.deepcopy(X),n=n_s,maxts = 7,mints=3,nmax=100,L0=1,UseP=1,verb=0,Init=0,Aposit=False,\
    ...
    for it1 in range(len(lineBlockSize)):

        time1 = time.time()
        Results_sB1 = blgmca4(cp.deepcopy(X),n=n_s,maxts = 7,mints=3,nmax=100,L0=1,UseP=1,verb=0,Init=0,Aposit=False,\
                            BlockSize= None,NoiseStd=[],IndNoise=[],Kmax=1.,AInit=None,tol=1e-6,subBlockSize=sizeBlock,\
                            lineBlockSize=lineBlockSize[it1],threshOpt=2,weightFMOpt=1,SCOpt=1,alphaEstOpt=0,optA=1,alpha_exp=alpha_init)
        ...
        time1 = time.time()
        Results_sB2 = blgmca2_bis(cp.deepcopy(X),n=n_s,maxts = 7,mints=3,nmax=100,L0=1,UseP=1,verb=0,Init=0,Aposit=False,\
                            BlockSize= None,NoiseStd=[],IndNoise=[],Kmax=1.,AInit=None,tol=1e-6,subBlockSize=sizeBlock,\
                            lineBlockSize=lineBlockSize[it1],threshOpt=2,weightFMOpt=1,SCOpt=1,alphaEstOpt=0,optA=1,alpha_exp=alpha_init)
        time_results[1,it_n] = time.time() - time1
        ...
        time1 = time.time()
        Results_sB3 = blgmca2_bisbis(cp.deepcopy(X),n=n_s,maxts = 7,mints=3,nmax=100,L0=1,UseP=1,verb=0,Init=0,Aposit=False,\
                            BlockSize= None,NoiseStd=[],IndNoise=[],Kmax=1.,AInit=None,tol=1e-6,subBlockSize=sizeBlock,\
                            lineBlockSize=lineBlockSize[it1],threshOpt=2,weightFMOpt=1,SCOpt=1,alphaEstOpt=0,optA=1,alpha_exp=alpha_init)
        time_results[1,it_n] = time.time() - time1
        A_sB3 = Results_sB3['mixmat']
        S_sB3 = Results_sB3['sources']
        crit_sB3 = EvalCriterion_fast(A0,S0,A_sB3,S_sB3) 
