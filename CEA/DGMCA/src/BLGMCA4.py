...
##----- Block loop
        for itB in range(numBlocks):
            ...
            # Used for the fusion of the AMCA weights
            wa_block = np.zeros([numLineBlocks,valB[iB+1]-valB[iB]])
...
#------------------ Udpating the AMCA block weights
                    if SCOpt == 1:
                        ...
                        wa = wa/np.max(wa)
                        wa_block[iL,:] = wa
                        # print('itB: %d / %d. wa:'%(itB,numBlocks))
                        # print wa
                        # print wa.shape
...
#---------- Agregate the wa weights: do the median only to nonzero values of the weighted average
                wa_final_block = np.multiply(np.reshape(w_Sj,(-1,1)),wa_block)
                wa_final = np.zeros([wa_final_block.shape[1]])
                for it_wa in range(wa_final_block.shape[1]):
                    aux_wa = wa_final_block[:,it_wa]
                    wa_final[it_wa] = np.median(aux_wa[abs(aux_wa)>1e-24])
