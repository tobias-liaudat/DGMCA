...
ATTENTION!!
READ THIS:
This code replaces the Frechet mean by a simple euclidean mean while maintaining all the other code.
It is just used for comparison.
---->
...
def DGMCA_euclideanSNR(X,n=2,mints=3,nmax=100,q_f=0.1,L0=0,verb=0,Init=0,BlockSize= None,\
...
#---------- Fusion method: The Frechet Mean
            ...
            # Euclidean mean
            for it_src in range(n):
                for it_nBlck in range(numBlocks):
                    A_block[:,it_src,it_nBlck] = A_block[:,it_src,it_nBlck] * w_FM[it_src,it_nBlck]


            A_mean_new = np.mean(A_block,axis=2)
            # print('A_mean BEFORE normalization')
            # print(A_mean)
            for it_n in range(A_mean_new.shape[1]):
                if np.sum(A_mean_new[:,it_n]) != 0:
                    A_mean_new[:,it_n] = A_mean_new[:,it_n]/(np.sqrt(np.sum(A_mean_new[:,it_n]**2)))
                else:
                    A_mean_new[:,it_n] = A_mean[:,it_n]
                    
            A_mean = A_mean_new
            # print('A_mean AFTER normalization')
            # print(A_mean)
