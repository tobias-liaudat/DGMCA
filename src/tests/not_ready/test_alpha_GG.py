...
alphas = np.array([ 0.1, 0.15, 0.25, 0.35, 0.5, 0.7, 1])
...
ca_final_sB = np.zeros([2,len(divisors),len(alphas),numIts]) # [i,:,:,:] i=0 mean i=1 med
ca_final_tot = np.zeros([2,len(alphas),numIts]) # [i,:,:] i=0 mean i=1 med


for it_n in range(len(alphas)):
#    n_s = n_sources[it_n]
#    n_obs = n_s
#    100 = n_epoch[it_n]
    alpha = alphas[it_n]
    ...
    for it0 in range(numIts):
        X,X0,A0,S0,N = Make_Experiment_GG(n_s=n_s,n_obs=n_obs,t_samp=totalSize,noise_level=60.0,dynamic=0,CondNumber=1,alpha=alpha)
        ...
        for it1 in range(len(divisors)):
            ...
            Results_sB = bgmca3(cp.deepcopy(X),n=n_s,maxts = 7,mints=3,nmax=100,L0=1,UseP=1,verb=0,Init=0,Aposit=False,\
                ...
                threshOpt=2,weightFMOpt=1,SCOpt=1,alphaEstOpt=1,optA=1,alpha_exp=2.)
...
title_str = "Test_alpha_GG_" + "_totalSize_" + str(totalSize) + "_minSizeBlock_" + str(minSizeBlock) +\
    "_numIts_" + str(numIts) + "_alphas_" + str(len(alphas))
